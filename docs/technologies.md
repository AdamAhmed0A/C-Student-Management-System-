...
| **Testing / Validation**     | **Qt Test (QTestLib)** – used in the _exam_ harness to run unit‑style checks on controllers/models.                   | Verifies that controllers correctly talk to the DB, that UI logic updates tables, etc.                                                                                                                                                                                                          | `QTEST_MAIN`, `QVERIFY`, `QCOMPARE`                                                                                                                            |
...
1. **Qt Widgets** – All UI screens (`AdminPanel`, `ProfessorPanel`, `StudentPanel`, etc.) inherit from `QWidget`.  
   _Signals_ (e.g., `clicked()`) and _slots_ (e.g., `onAddStudent()`) connect UI actions to controller logic.
